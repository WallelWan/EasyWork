...
- **类型系统**：`Packet` + `std::any` 载荷与 `TypeInfo` 反射，连接时检查与预编译转换。
...
5. **Process**：数据处理循环（由框架自动调度的 `RunDispatch`）。
6. **Close()**：资源释放（用户可重写）。
...
数据在节点间以 `Packet` 形式传递：
- **Payload**：`std::shared_ptr<std::any>`，支持零拷贝扇出。
...
EasyWork 基于 C++ 方法签名进行类型登记，Python 输入与节点间连接在构建期自动生成转换器。

- **无手动类型清单**：支持类型来自方法/构造函数签名登记。
- **预编译转换**：连接时注入 `AnyCaster`，运行时无分支。
- **自定义对象**：C++ 类型只要被方法签名引用即可被自动注册。
...
- `ew.get_method_dispatch_counts()`：获取方法调度统计（left/right/forward 计数）。
- `ew.get_method_dispatch_order_errors()`：获取调度顺序错误计数（用于调试 `set_method_order`）。
- `ew.get_small_tracked_live_count()`：监控对象生命周期（SBO 优化测试）。
...
### 7.1 SyncBarrier (仅限 C++)

`SyncBarrier` 是一个特殊的内置节点，用于多路输入的时间戳对齐。它会缓存多个端口的输入，直到找到时间戳在容差范围内的一组数据，打包成 Tuple 输出。当前仅支持在 C++ 侧使用。
...
- **AST 解析与原生控制流**：实现 AST 分析（类似于 Numba/Triton），以支持在构图中使用原生的 Python `if/else`, `for/while` 语法，替代对专用流控制节点的依赖。
